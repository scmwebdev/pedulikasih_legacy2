...
class Content_model extends CI_Model {
    ...
    function submitData() {
    		$judul_url = $this->allfunction->judul2url($this->input->post('judul'));
				...
				$upload_dir = STATIC_PATH.'pdf/kitapeduli/content/';
				...
				if ($data_id == "") {
						$this->db->insert('kitapeduli_content', $data);
						//$data_id = $this->db->insert_id();
				} else {
						$this->db->where('id', $data_id);
						$this->db->update('kitapeduli_content', $data);
				}
		}
		
		function deleteData($data_id) {
				$sql = "delete from kitapeduli_content where id=$data_id";
				$this->db->query($sql);
		}

		function publishData($data_id,$set) {
				$sql = "update kitapeduli_content set status=$set where id=$data_id";
				$this->db->query($sql);
		}
		
    function getData($data_id) {
				$sql = "select * from kitapeduli_content where id=$data_id";
		...
		function json() {
				$data 	= array();
				$sql 		= "select * from kitapeduli_content order by id desc";
				$query 	= $this->db->query($sql);
