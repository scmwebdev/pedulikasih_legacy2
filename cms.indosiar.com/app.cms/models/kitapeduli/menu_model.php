...
class Menu_model extends CI_Model {
    ...
    function submitData() {
    		$judul_url = $this->allfunction->judul2url($this->input->post('judul_menu'));
				...
				$upload_dir = STATIC_PATH.'pdf/kitapeduli/';
				...
				if ($data_id == "") {
						$this->db->insert('kitapeduli_menu', $data);
						//$data_id = $this->db->insert_id();
				} else {
						$this->db->where('id', $data_id);
						$this->db->update('kitapeduli_menu', $data);
				}
		}
		
		function deleteData($data_id) {
				$total = $this->db->where('id_main',$data_id)->count_all_results('kitapeduli_menu');
				if ($total == 0) {
						$sql = "delete from kitapeduli_menu where id=$data_id";
						$this->db->query($sql);
				}
		}

		function publishData($data_id,$set) {
				$sql = "update kitapeduli_menu set status=$set where id=$data_id";
				$this->db->query($sql);
		}
		
    function getData($data_id) {
				$sql = "select * from kitapeduli_menu where id=$data_id";
		...
		function json() {
				$data = array();
				$sql = "select * from kitapeduli_menu where id_main=0 order by status_sort";
				$query = $this->db->query($sql);
				foreach($query->result_array() as $row) {
						$row['judul_menu'] = '<b>'.$row['judul_menu'].'</b>';
						$data[] = $row;
						
						$sqlx = "select * from kitapeduli_menu where id_main=".$row['id']." order by status_sort";
						$queryx = $this->db->query($sqlx);
						foreach($queryx->result_array() as $rowx) {
								$rowx['judul_menu'] = '- '.$rowx['judul_menu'];
								$data[] = $rowx;
								
								$sqlz = "select * from kitapeduli_menu where id_main=".$rowx['id']." order by status_sort";
		...
		function jsonmainmenu() {
	    	...
	    	$data[] = $row;
				...
				$sql = "select id,judul_menu from kitapeduli_menu where id_main=0 order by status_sort";
				$query = $this->db->query($sql);
				foreach($query->result_array() as $row) {
						$data[] = $row;
						
						$sqlx = "select id,judul_menu from kitapeduli_menu where id_main=".$row['id']." order by status_sort";
						$queryx = $this->db->query($sqlx);
						foreach($queryx->result_array() as $rowx) {
								$rowx['judul_menu'] = '- '.$rowx['judul_menu'];
								$data[] = $rowx;
