...
class Content_model extends CI_Model {
    ...
    function submitData() {
    		$judul_url = $this->allfunction->judul2url($this->input->post('judul'));
				...
				if ($data_id == "") {
						$this->db->insert('pedulikasih_content', $data);
						//$data_id = $this->db->insert_id();
				} else {
						$this->db->where('id', $data_id);
						$this->db->update('pedulikasih_content', $data);
				}
		}
		
		function deleteData($data_id) {
				$sql = "delete from pedulikasih_content where id=$data_id";
				$this->db->query($sql);
		}

		function publishData($data_id,$set) {
				$sql = "update pedulikasih_content set status=$set where id=$data_id";
				$this->db->query($sql);
		}
		
    function getData($data_id) {
				$sql = "select * from pedulikasih_content where id=$data_id";
		...
		function json() {
				$data 	= array();
				$sql 		= "select * from pedulikasih_content order by id desc";
				$query 	= $this->db->query($sql);
