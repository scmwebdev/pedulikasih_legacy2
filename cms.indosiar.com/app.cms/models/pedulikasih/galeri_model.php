...
class Galeri_model extends CI_Model {
    ...
    function submitData() {
		...
		if ($data_id == "") {
			$this->db->insert('pedulikasih_album', $data);
			//$data_id = $this->db->insert_id();
		} else {
			$this->db->where('id', $data_id);
			$this->db->update('pedulikasih_album', $data);
		}
	}
	...
	function submitImage() {
		...
		if ($data_id == "") {
			$this->db->insert('pedulikasih_album_foto', $data);
		} else {
			$this->db->where('id', $data_id);
			$this->db->update('pedulikasih_album_foto', $data);
		}
	}
    ...
    function submitVideo() {
		...
		if ($data_id == "") {
			$this->db->insert('pedulikasih_album_video', $data);
		} else {
			$this->db->where('id', $data_id);
			$this->db->update('pedulikasih_album_video', $data);
		}
	}
	
	function deleteData($data_id) {
		$total = $this->db->where('id_album',$data_id)->count_all_results('pedulikasih_album_foto');
		if ($total == 0) {
			$sql = "delete from pedulikasih_album where id=$data_id";
			$this->db->query($sql);
		}
	}

	function deleteImage($data_id) {
		$sql = "delete from pedulikasih_album_foto where id=$data_id";
		$this->db->query($sql);
	}
	
	function deleteVideo($data_id) {
		$sql = "delete from pedulikasih_album_video where id=$data_id";
		$this->db->query($sql);
	}
	
	function publishData($data_id,$set) {
		$sql = "update pedulikasih_album set status=$set where id=$data_id";
		$this->db->query($sql);
	}
	
	function getData($data_id) {
		$sql 	= "select * from pedulikasih_album where id=$data_id";
	...
	function getImage($data_id) {
		$sql 	= "select * from pedulikasih_album_foto where id=$data_id";
		$query 	= $this->db->query($sql);
		$data 	= $query->row_array();				
		$query->free_result();
	...
	function getVideo($data_id) {
		$sql 	= "select * from pedulikasih_album_video where id=$data_id";
		$query 	= $this->db->query($sql);
		$data 	= $query->row_array();				
		$query->free_result();
	...
	function json() {
		$data 	= array();
		$sql 	= "select * from pedulikasih_album order by tanggal desc";
		$query 	= $this->db->query($sql);
