...
class Photo_model extends CI_Model {
    ...
    function json($kategori,$keyword,$start,$limit,$page) {
        ...
        if ($keyword == "")
            $sql = "select count(id) as total from photo WHERE publish=1 $kategori";
        else
            $sql = "select count(id) as total from photo where publish=1 AND judul like '%$keyword%' $kategori";
        ...
        if ($query->num_rows() == 0) {
            ...
            if ($keyword == "")
                $sql = "select * from photo WHERE publish=1 $kategori order by id desc limit $start,$limit";
            else
                $sql = "select * from photo where publish=1 AND judul like '%$keyword%' $kategori order by id desc limit $start,$limit";
    ...
    function get_kategori() {
        return $this->DBW->order_by("sort")->get_where('photo_kategori', array('publish' => 1))->result_array();
    }
    ...
    function submit_data($data) {
		...
		$upload_dir = STATIC_PATH.'thevoiceindonesia/photo/';
        ...
        if($data_id == '') {
            $this->DBW->insert('photo', $data);
        } else {
            $this->DBW->where('id', $data_id);
            $this->DBW->update('photo', $data); 
        }
    ...
    function deletedata($data_id) {
        $sql = "delete from photo where id=$data_id";
        $this->DBW->query($sql);
    }
		
    function getData($data_id) {
        if (is_numeric($data_id)) {
            $sql = "select * from photo where id=$data_id";
            $query = $this->DBW->query($sql);
