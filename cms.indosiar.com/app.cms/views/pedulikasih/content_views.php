...
Ext.onReady(function(){			
	  ...
	  function renderTick(val) {
		...
		var gridData = Ext.create('Ext.grid.Panel', {
		    store: storeData,
		    width: '100%',
		    title: 'Peduli Kasih Content Management',
		    region: 'center',
		    columnLines: true,
		    ...
		    columns: [    
    			...
    			{text:'Judul', dataIndex:'judul', sortable: false, width:250},
                    ...
                    handler: function(grid, rowIndex, colIndex) {
                        ...
                        if (rec.get("pdf") != "") window.open('<?=STATIC_URL?>pdf/pedulikasih/content/'+rec.get("pdf"));
                    }
		          	}
	          	]
					},
	          ...
	          items: [
			          {
			              ...
			              handler: function(grid, rowIndex, colIndex) {
			                  var rec = gridData.getStore().getAt(rowIndex);
			                  var uri = 'http://www.indosiar.com/pedulikasih/'+rec.get("kategori")+'/'+rec.get("judul_url");
			                  window.open(uri);
			              }
			          },
			          <?php if (isset($modAuths['edit'])):?>
