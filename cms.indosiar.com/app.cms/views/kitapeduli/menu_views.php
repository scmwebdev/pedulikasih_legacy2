...
Ext.onReady(function(){			
	  ...
	  function renderTick(val) {
		...
		var gridData = Ext.create('Ext.grid.Panel', {
		    store: storeData,
		    width: '100%',
		    title: 'Kita Peduli Menu Management',
		    region: 'center',
		    columnLines: true,
		    ...
		    columns: [    
    			...
    			{text:'URL', dataIndex:'url', sortable: false, width:80, align:'center'},
                    ...
                    handler: function(grid, rowIndex, colIndex) {
                        ...
                        if (rec.get("pdf") != "") window.open('<?=STATIC_URL?>pdf/kitapeduli/'+rec.get("pdf"));
                    }
		          	}
	          	]
					},
	          ...
	          items: [
			          {
			              ...
			              handler: function(grid, rowIndex, colIndex) {
			                  var rec = gridData.getStore().getAt(rowIndex);
			                  var uri = 'http://www.indosiar.com/kitapeduli/'+rec.get("judul_url");
			                  if (rec.get("pdf") != "") uri = '<?=STATIC_URL?>pdf/kitapeduli/'+rec.get("pdf");
			                  if (rec.get("url") != "") uri = 'http://www.indosiar.com/kitapeduli/'+rec.get("url");
			                  window.open(uri);
			              }
			          },
			          <?php if (isset($modAuths['edit'])):?>
