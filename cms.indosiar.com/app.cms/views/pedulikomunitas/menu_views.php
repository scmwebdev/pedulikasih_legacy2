...
Ext.onReady(function(){			
	  ...
	  function renderTick(val) {
		...
		Ext.define('modelData', {
		    ...
		    fields: ['id','id_main','judul','judul_menu','judul_url','url','pdf','isi','status_sort','tanggal','publish']
		});
		...
		var gridData = Ext.create('Ext.grid.Panel', {
		    store: storeData,
		    width: '100%',
		    title: 'Peduli Komunitas Menu Management',
		    region: 'center',
		    columnLines: true,
	      selModel: Ext.create('Ext.selection.CheckboxModel', {
		    ...
		    listeners : {
				...
				<?php endif;?>
		    columns: [    
					{text:'Menu', dataIndex:'judul_menu', sortable: false, width:150},
    			{text:'Judul', dataIndex:'judul', sortable: false, width:150},
    			{text:'URL', dataIndex:'url', sortable: false, width:80, align:'center'},
					{
			    		xtype: "actioncolumn", 
			    		header:'PDF',
			    		align:'center', 
                    ...
                    handler: function(grid, rowIndex, colIndex) {
                        ...
                        if (rec.get("pdf") != "") window.open('<?=STATIC_URL?>pdf/pedulikomunitas/'+rec.get("pdf"));
                    }
		          	}
	          	]
					},
	          ...
	          items: [
			          {
			              ...
			              handler: function(grid, rowIndex, colIndex) {
			                  var rec = gridData.getStore().getAt(rowIndex);
			                  var uri = 'http://www.indosiar.com/pedulikomunitas/'+rec.get("judul_url");
			                  if (rec.get("pdf") != "") uri = '<?=STATIC_URL?>pdf/pedulikomunitas/'+rec.get("pdf");
			                  if (rec.get("url") != "") uri = 'http://www.indosiar.com/pedulikomunitas/'+rec.get("url");
			                  window.open(uri);
			              }
			          },
			          <?php if (isset($modAuths['edit'])):?>
