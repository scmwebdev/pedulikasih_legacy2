...
function Scroller(x, y, width, height, border, padding) {
  ...
  this.setDirection = scrollerSetDirection;
  this.addItem = scrollerAddItem;
  this.setItem = scrollerSetItem;
  ...
  this.next = scrollerNext;
  this.prev = scrollerPrev;
}
...
function scrollerSetDirection(dir) {
...
function scrollerSetItem(index,str) {
...
function scrollerCreate() {
  ...
  if (isMinIE4) {
    ...
    str = '<div id="scroller' + i + '_baseLayer"'
        ...
        + 'xbackground-color:' + this.bdColor + ';'
        + 'width:' + this.width + 'px;'
        ...
        + ' style="position:absolute;'
        + 'xbackground-color: ' + this.bgColor + ';'
        + 'width:' + this.width + 'px;'
...
//window.status = scrollerList[i].currentX + "^" + scrollerList[i].nextX;
		  scrollLayerTo(scrollerList[i].scrollLayer,
		                Math.round(scrollerList[i].currentX),0,
		                false);
		}
		else {
...
//window.status = scrollerList[i].currentY + "^" + scrollerList[i].nextY;
		  scrollLayerTo(scrollerList[i].scrollLayer,
...
function scrollerMove(dir) {
  ...
  for (i = 0; i < scrollerList.length; i++) {
	...
	if(scrollerList[i].direction == 1) {
		...
		{
			scrollerList[i].currentX = scrollerList[i].nextX + - 2*scrollerList[i].width;
		    scrollerList[i].nextX -= scrollerList[i].width;
		}
		scrollerList[i].paused = true;
		scrollerList[i].counter = 0;
	  ...
	  if (scrollerList[i].currentX <0) {
	    scrollerList[i].currentX += scrollerList[i].maxX;
	    scrollerList[i].nextX = scrollerList[i].maxX;
	  }
//window.status = scrollerList[i].currentX + "-" + scrollerList[i].nextX;
	  scrollLayerTo(scrollerList[i].scrollLayer,
	                Math.round(scrollerList[i].currentX),0,
		...
		{
			scrollerList[i].currentY = scrollerList[i].nextY - 2*scrollerList[i].height;
		    scrollerList[i].nextY -= scrollerList[i].height;
		}
	    scrollerList[i].paused = true;
	    scrollerList[i].counter = 0;
	  ...
	  if (scrollerList[i].currentY <0) {
	    scrollerList[i].currentY += scrollerList[i].maxY;
	    scrollerList[i].nextY = scrollerList[i].maxY;
	  }
//window.status = scrollerList[i].currentY + "-" + scrollerList[i].nextY;
	  scrollLayerTo(scrollerList[i].scrollLayer,
	                0, Math.round(scrollerList[i].currentY),
...
function scrollerReload() {
  ...
  if (isMinIE4)
  {
    ...
    idTimer = setTimeout('window.location.href = window.location.href', 1000);
  }
  else
