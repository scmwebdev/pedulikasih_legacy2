...
class Pedulikasih_model extends CI_Model {
    ...
    function pagingbca($iNum,$sql,$totrecord,$batas,$segment,$totpenyumbang) {     
			$config['base_url'] = site_url("pedulikasih/bcaperorangan/page");
		  ...
		  <table width="100%" border="0" cellspacing="1" cellpadding="3" style="border:1px solid #333">
			...
			$query = $this->pedulikasih_model->looprecord($sql);
    ...
    function showMenux() {
		...
		if ($queryx->num_rows() > 0) {
			...
			foreach ($queryx->result_array() as $rowx) {
				...
				if ($rowx['pdf'] != "") $link = URL_STATIC.'assets/pdf/pedulikasih/'.$rowx['pdf'];
				// if ($rowx['pdf'] != "") $link = base_url() .'/assets/pdf/pedulikasih/'.$rowx['pdf'];
				if ($rowx['url'] != "") $link = '/pedulikasih/'.$rowx['url'];
				
				if ($rowx['judul_url'] == "pemohon")
				    $str .= '<li><a href="#">'.$rowx['judul_menu'].'</a></li>';
				else
				    $str .= '<li><a href="'.$link.'">'.$rowx['judul_menu'].'</a></li>';
						
				$sqlz = "select * from pedulikasih_menu where publish=1 and id_main=".$rowx['id']." order by status_sort";
				$queryz = $this->db->query($sqlz);
				foreach ($queryz->result_array() as $rowz) {
					$link = '/pedulikasih/'.$rowz['judul_url'];
					if ($rowz['pdf'] != "") $link = URL_STATIC.'assets/pdf/pedulikasih/'.$rowz['pdf'];
					// if ($rowx['pdf'] != "") $link = base_url() .'/assets/pdf/pedulikasih/'.$rowx['pdf'];
    ...
    function showMenu() {
		...
		if ($queryx->num_rows() > 0) {			
			foreach ($queryx->result_array() as $rowx) {
				...
				if ($rowx['pdf'] != "") $link = base_url().'asset/pedulikasih/'.$rowx['pdf'];
				if ($rowx['url'] != "") $link = '/pedulikasih/'.$rowx['url'];
    ...
    function showDonatur($iNum,$sql,$totrecord,$batas,$segment,$totpenyumbang) {     
		$config['base_url'] = site_url("pedulikasih/donatur/page");
		$config['total_rows'] = $totrecord;
		$config['per_page'] = $batas;
		$config['uri_segment'] = $segment;
		$config['num_links'] = 5;
	...
	function getMenuDetail($judul_url,$url="") {
		...
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql 	= "select * from pedulikasih_menu where publish=1 and ".(($url == "") ? "judul_url='$judul_url'" : "url='$url'")." limit 1";
				$query 	= $this->db->query($sql);
				$data 	= $query->row_array();
				$query->free_result();
	...
	function getContentList($kategori,$limit=10,$excID="") {
		$data = array();
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql	= "select judul,judul_url,ringkasan,kategori,pdf from pedulikasih_content where kategori='$kategori' and publish=1 ".($excID == "" ? "" : "and id<>$excID")." order by id desc limit $limit";
				$query 	= $this->db->query($sql);
				$data 	= $query->result_array();
				$query->free_result();
	...
	function getContentDetail($kategori,$judul_url) {
		$data = array();
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql 	= "select * from pedulikasih_content where kategori='$kategori' and judul_url='$judul_url' limit 1";
				$query 	= $this->db->query($sql);
				$data 	= $query->row_array();
				$query->free_result();
	...
	function getAlbumDetail($judul_url) {
		$sql 	= "select * from pedulikasih_album where judul_url='$judul_url' limit 1";
		return $this->fetchRowArray($sql);
	}
	
	function getPhotoList($id_album) {
		$sql 	= "select * from pedulikasih_album_foto where id_album=$id_album order by urutan asc";
		return $this->fetchResultArray($sql);
	}
	
	function getLastPhoto($id_album) {
		$sql 	= "select * from pedulikasih_album_foto where id_album=$id_album order by urutan asc limit 1";
		return $this->fetchRowArray($sql);
	}
	
	function getVideoList($id_album) {
		$sql 	= "select * from pedulikasih_album_video where id_album=$id_album order by urutan asc";
		return $this->fetchResultArray($sql);
	}
}
