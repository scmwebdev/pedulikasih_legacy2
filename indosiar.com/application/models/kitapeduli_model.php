...
class Kitapeduli_model extends CI_Model {
    ...
    function pagingrs($url,$sql,$totrecord,$batas,$segment,$content="intro") {    
			...
			if ($totrecord>0) {
				...
				$paging.='
					<div class="ContentJenisList RoundedBox8px">
						<h2>'.$row->title.'</a></h2>'.
						str_replace("http://www.indosiar.com/pdf/kitapeduli/","http://static.indosiar.com/pdf/kitapeduli/",$row->introtext).'
						<div style="clear:both"></div>
					</div>';		
				}	
			}	
    ...
    function showMenux() {
		...
		$sqlx = "select * from kitapeduli_menu where publish=1 and id_main=0 order by status_sort";
		$queryx = $this->db->query($sqlx);
		if ($queryx->num_rows() > 0) {
			$str .= '<ul>';
			
			foreach ($queryx->result_array() as $rowx) {
				$link = '/kitapeduli/'.$rowx['judul_url'];
				if ($rowx['pdf'] != "") $link = URL_STATIC.'pdf/kitapeduli/'.$rowx['pdf'];
				if ($rowx['url'] != "") $link = '/kitapeduli/'.$rowx['url'];
				...
				$sqlz = "select * from kitapeduli_menu where publish=1 and id_main=".$rowx['id']." order by status_sort";
				$queryz = $this->db->query($sqlz);
				foreach ($queryz->result_array() as $rowz) {
					$link = '/kitapeduli/'.$rowz['judul_url'];
					if ($rowz['pdf'] != "") $link = URL_STATIC.'pdf/kitapeduli/'.$rowz['pdf'];
					if ($rowz['url'] != "") $link = '/kitapeduli/'.$rowz['url'];
    ...
    function showDonatur($iNum,$sql,$totrecord,$batas,$segment,$totpenyumbang) {     
		$config['base_url'] = site_url("kitapeduli/donatur/page");
		$config['total_rows'] = $totrecord;
		$config['per_page'] = $batas;
		$config['uri_segment'] = $segment;
		$config['num_links'] = 5;
	...
	function getMenuDetail($judul_url,$url="") {
		...
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql 	= "select * from kitapeduli_menu where publish=1 and ".(($url == "") ? "judul_url='$judul_url'" : "url='$url'")." limit 1";
				$query 	= $this->db->query($sql);
				$data 	= $query->row_array();
				$query->free_result();
	...
	function getContentList($kategori,$limit=10,$excID="") {
		$data = array();
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql	= "select judul,judul_url,ringkasan,kategori,pdf from kitapeduli_content where kategori='$kategori' and publish=1 ".($excID == "" ? "" : "and id<>$excID")." order by id desc limit $limit";
				$query 	= $this->db->query($sql);
				$data 	= $query->result_array();
				$query->free_result();
	...
	function getContentDetail($kategori,$judul_url) {
		$data = array();
		//if (!$data = $this->ivmcache->get('pk:menu:'.$judul_url)) {
				$sql 	= "select * from kitapeduli_content where kategori='$kategori' and judul_url='$judul_url' limit 1";
				$query 	= $this->db->query($sql);
				$data 	= $query->row_array();
				$query->free_result();
	...
	function getLastPhoto($id_album) {
		$sql 	= "select * from kitapeduli_album_foto where id_album=$id_album order by urutan asc limit 1";
		return $this->fetchRowArray($sql);
	}
