...
class Register extends CI_Controller {
	...
	function index()
	{	
		$data['HTMLPageTitle'] = "Registrasi - Take Me Out";
		$this->load->view('takemeout/takemeout_header', $data);
		$this->load->view('takemeout/takemeout_register', $data);
	...
	{		
		if ($this->input->post('nama_lengkap', TRUE)) {
			...
			$foto_dir 	= "/san/static/indosiar.com/takemeout/registrasi/";
			$FILE_MIMES = array('image/jpeg','image/jpg','image/png');         
			$FILE_EXTS  = array('.jpeg','.jpg','.png');
			
			//if (!file_exists("/san/static/indosiar.com/takemeout")) mkdir("/san/static/indosiar.com/takemeout");
			//if (!file_exists("/san/static/indosiar.com/takemeout/registrasi")) mkdir("/san/static/indosiar.com/takemeout/registrasi");
			...
			foreach($this->db->list_fields('takemeout_registrasi') as $row) if (isset($_POST[$row])) $data[$row] = trim($_POST[$row]);
			...
			$this->db->insert('takemeout_registrasi', $data);
