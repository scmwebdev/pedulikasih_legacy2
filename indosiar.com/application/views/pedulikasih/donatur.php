...
<h1>Daftar Donatur Peduli Kasih</h1>
<p>&nbsp;</p>
<div class="csr_subtitle">PENCARIAN</div>
<div style="width:49%;float:left;">
    <form action="<?php site_url('pedulikasih/donatur')?>" method=post name="form1">
...
<div style="width:49%;float:right;">
    <form action="<?php site_url('pedulikasih/donatur')?>" method=post name="form2">
    <input type="hidden" name="kategori" value="<?php $kategori?>" />
...
$sqltot = "select ID from pedulikasih_bca where kategori='$kategori'";
$totpenyumbang=$this->pedulikasih_model->totalrecord($sqltot);
...
if ($que == 1) {
  ...
  $qword = mysqli_real_escape_string($db, $qword);
  $jenis = mysqli_real_escape_string($db, $jenis);

  if ($jenis == "kota") {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and kota like '%$qword%'";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and kota like '%$qword%' order by tanggal desc ".$strlimit;
  } elseif ($jenis == "nama") {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and nama like '%$qword%'";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and nama like '%$qword%' order by tanggal desc ".$strlimit;
  } else {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and (nama like '%$qword%' or kota like '%$qword%')";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and (nama like '%$qword%' or kota like '%$qword%') order by tanggal desc ".$strlimit;
  }
} elseif ($que == 2) {
  ...
  if ($satu != "" && $dua == "") {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and nilai between $satu and 300000000";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and nilai between $satu and 300000000 order by nilai desc".$strlimit;
  } elseif ($satu == "" && $dua != "") {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and nilai between 0 and $dua";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and nilai between 0 and $dua order by nilai desc".$strlimit;
  } elseif ($satu != "" && $dua != "") {
    $sqltot = "select id from pedulikasih_bca where kategori='$kategori' and nilai between $satu and $dua";
    $sql = "select * from pedulikasih_bca where kategori='$kategori' and nilai between $satu and $dua order by nilai desc".$strlimit;
  }
}
