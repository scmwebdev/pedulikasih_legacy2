	...
	if (count($pk_list) > 0) {
			...
			foreach($pk_list as $row) {
				if ($pk_kategori == "audit" && $row['pdf'] != "")
					echo '<li><a href="'.URL_STATIC.'pdf/pedulikomunitas/content/'.$row['pdf'].'">'.$row['judul'].'</a>'.$row['ringkasan'].'</li>';
				else
					echo '<li><a href="/pedulikomunitas/'.$row['kategori'].'/'.$row['judul_url'].'">'.$row['judul'].'</a>'.substr($row['ringkasan'],0,200).'...</li>';
			}
