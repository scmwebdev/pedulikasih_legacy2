...
if (count($pk_data) > 0) {
	...
	if (count($pk_foto) > 0) {
		...
		foreach($pk_foto as $row) echo '<li><a class="fancybox-button" rel="fancybox-button" href="'.$this->config->item('URL_IMAGES').'pedulikomunitas/gallery/'.$row['image'].'" title="'.str_replace(array("\r","\n"),"",htmlspecialchars(strip_tags($row['keterangan']))).'"><img src="'.$this->config->item('URL_IMAGES').'pedulikomunitas/gallery/th_'.$row['image'].'" width="150" border="0" alt="" /></a></li>';
		echo '</ul></div>
		<div style="clear:both"></div>';
	}
	...
	echo '
	<p>&nbsp;</p>
	<div style="text-align:right"><span class="csr_clickhere" style="margin-bottom:0px;"><a href="'.site_url('pedulikomunitas/gallery').'">BACK</a></span></div>
	';
