...
if ($this->uri->segment(2)!="") {
	include (APPPATH."views/videofiesta/artikel.php");
}else {
...
<table width="996" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="15" height="10" /></td>
    <td>
    ...
    <td><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="16" height="10" /></td>
    <td>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="22" background="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>bg_bar.gif">
  ...
  <tr>
    <td colspan="5"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="10" height="10" /></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td valign="top">
          ...
          <td>
            <div align="center" style="margin-bottom:10px; background-color:#333333">
			        ...
			        <script type="text/javascript" src="http://www.indosiar.com/swf/viral/swfobject.js"></script>
							...
							var so = new SWFObject('http://www.indosiar.com/swf/jw/player.swf','mpl','400','320','9','#ffffff');
...
<?
$rec=1;
$sql="select tblv.*,tblk.kategori from tbl_video as tblv inner join tbl_video_kategori as tblk on tblv.id_kategori=tblk.id and tblv.id_kategori<>11 and ".$sql_not1." and tblv.judul<>'' order by tblv.id desc limit 0,6";
$query = $this->db->query($sql);
if ($query->num_rows() > 0) {
	$sql_not.=" and ";
	$sql_not1.=" and ";	
...
?>			
				          <a href="<?=$this->config->item('URL_VIDEOFIESTA')?>kiss/0/<?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?><?=$row->image?>" width="139" height="64" border="0" align="left" /></a>

				          <span class="judul"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?>kiss/0/<?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><?=$row->judul?></a> </span><br />         
<?			} else {
?>
				          <a href="<?=$this->config->item('URL_VIDEOFIESTA')?><?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?><?=$row->image?>" width="139" height="64" border="0" align="left" /></a>
				          <span class="judul"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?><?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><?=$row->judul?></a> </span><br />         		
<?			}	
?>				          
...
<?
$rec=1;
$sql="select tblv.*,tblk.kategori from tbl_video as tblv inner join tbl_video_kategori as tblk on tblv.id_kategori=tblk.id and tblv.id_kategori<>11 and ".$sql_not1." and tblv.judul<>'' order by tblv.id desc limit 0,7";
$query = $this->db->query($sql);
if ($query->num_rows() > 0) {
?>
	<div id="screen" >
		<img class="prev" src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>prev.gif" style="cursor:hand" alt="prev" width="42" height="53" />
		<div id="sections" style="height:400px">
			<ul>
<?	
	$sql_not.=" and ";
	$sql_not1.=" and ";
...
?>	
					...
					?><br>
<!--					Post: <?=$this->allfunction->beda_waktu(strtotime($row->tanggal))?> ago								          -->
				</li>
<?
				if ($rec<>7) {
					$sql_not1.=" and ";
...
?>
		...
		<img class="next" src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>next.gif" style="cursor:hand" alt="next" width="42" height="53" />
	</div>
<?
}
$query->free_result();				
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="22" background="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>bg_bar.gif">
      ...
      <img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="10" height="10" />
<?
$rec=1;
$sql="select tblv.*,tblk.kategori from tbl_video as tblv inner join tbl_video_kategori as tblk on tblv.id_kategori=tblk.id and tblv.id_kategori<>11 and ".$sql_not1." and tblv.judul<>'' order by tblv.id desc limit 0,8";
$query = $this->db->query($sql);
if ($query->num_rows() > 0) {
			$sql_not.=" and ";
			$sql_not1.=" and ";
...
<?
				if ($row->id_kategori==9) {
?>	
          <td width="139"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?>kiss/0/<?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?><?=$row->image?>" width="139" height="64" border="0" /></a></td>
          <td width="10"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="10" height="10" /></td>
          <td width="260"><span class="judul"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?>kiss/0/<?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><?=$row->judul?></a> </span><br /><!--Post: <?=$this->allfunction->beda_waktu(strtotime($row->tanggal))?> ago--></td>
<?			} else {
?>
          <td width="139"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?><?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?><?=$row->image?>" width="139" height="64" border="0" /></a></td>
          <td width="10"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="10" height="10" /></td>
          <td width="260"><span class="judul"><a href="<?=$this->config->item('URL_VIDEOFIESTA')?><?=$row->id?>/<?=$this->allfunction->judul2url(strip_tags($row->judul))?>"><?=$row->judul?></a> </span><br /><!--Post: <?=$this->allfunction->beda_waktu(strtotime($row->tanggal))?> ago--></td>
<?			}
?>	          	
          <td width="10"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="10" height="10" /></td>
...
<table width="996" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td colspan="5"><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="15" height="10" /><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="16" height="10" /><img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="15" height="10" /></td>
  </tr>
  <tr>
...
<?
$rec=1;
$query = $this->db->query("select tblv.*,tblk.kategori from tbl_video as tblv inner join tbl_video_kategori as tblk on tblv.id_kategori=tblk.id and tblv.id_kategori<>11 where ".$sql_not1." and tblv.judul<>'' order by tblv.id desc limit 0,4");
if ($query->num_rows() > 0) {
			$sql_not.=" and ";
			$sql_not1.=" and ";			
			$rows = $query->result();
...
?> 	
	...
	<img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>blnk.gif" width="16" height="10" />	 
<?
...
if ($query->num_rows() > 0) {
			...
			{						
				...
				if ($queryx->num_rows() > 0) {			
					$rowx = $queryx->row();			
?>
                        <div><a href="<?=$this->config->item('URL_VIDEOFIESTA')?><?=$rowx->id?>/<?=$this->allfunction->judul2url(strip_tags($rowx->judul))?>">
                          <?=$rowx->judul?>
                          </a> Comments:
                          <?=$row->jumlah?>
                          &nbsp;<img src="<?=$this->config->item('URL_VIDEOFIESTA_IMG')?>comment_icon_white.png" /> </div>
                      <?
				}	
				$queryx->free_result();				
			}
...
?>  	
    ...
    <td width="518" valign="top">
<?
$query = $this->db->query("select tblv.*,tblk.kategori from tbl_video as tblv inner join tbl_video_kategori as tblk on tblv.id_kategori=tblk.id and tblv.id_kategori<>11 where ".$sql_not1." and tblv.judul<>'' order by  tblv.id desc limit 0,7");
if ($query->num_rows() > 0) {
			$sql_not.=" and ";
			$sql_not1.=" and ";			
			$rows = $query->result();
